...
if [ "$OWNER" != "root:root" ]; then
    echo "FAIL: /var/www/html ownership is $OWNER, should be root:root"
    echo "HINT: Use 'sudo chown -R root:root /var/www/html' to fix ownership"
    exit 1
fi
...
if [ "$SELINUX_CONTEXT" != "httpd_sys_content_t" ]; then
    echo "FAIL: /var/www/html SELinux context is $SELINUX_CONTEXT, should be httpd_sys_content_t"
    echo "HINT: Use 'sudo restorecon -vFR /var/www/' to restore correct SELinux contexts"
    exit 1
fi
...
if [ "$HTTP_RESPONSE" != "200" ]; then
    echo "FAIL: Web server returned HTTP $HTTP_RESPONSE instead of 200"
    echo "HINT: Check if httpd is running with 'systemctl status httpd'"
    exit 1
fi
...
if [ "$WEB_CONTENT" -eq 0 ]; then
    echo "FAIL: Web server is running but serving incorrect content"
    echo "HINT: Verify /var/www/html/index.html exists and has correct permissions and SELinux context"
    exit 1
fi

echo "PASS: All checks completed successfully!"
echo "- Ownership is correct (root:root)"
echo "- SELinux context is correct (httpd_sys_content_t)"
echo "- Web server is responding with correct content"
